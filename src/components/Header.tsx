...
export default function Header() {
  ...
  return (
    <>
      <header className="w-full z-50 bg-gray-900/95 backdrop-blur-md border-b border-gray-800/50 sticky top-0 transition-all duration-300 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="relative px-4 py-2 text-gray-300 font-medium border-2 border-transparent hover:border-blue-500/50 hover:text-blue-400 rounded-xl transition-all duration-300 hover:scale-105 group"
                >
                  {link.name}
            ...
            <button
              ...
              className="md:hidden p-2 rounded-xl hover:bg-gray-800/80 transition-all duration-200"
      ...
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-gray-900/95 backdrop-blur-md border-l border-gray-800/50 z-50 transform transition-all duration-300 ease-in-out md:hidden shadow-2xl ${
          ...
          <nav className="flex flex-col space-y-6">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={() => setMobileMenuOpen(false)}
                className="px-4 py-3 text-gray-300 border-2 border-transparent hover:border-blue-500/50 hover:text-blue-400 rounded-xl transition-all duration-300 text-lg font-medium hover:scale-105"
              >
                {link.name}
              </a>
            ))}
